    ...
    public function getUserCart(Request $request)
    {
        ...
        foreach ($cart_data as $key => $cart) {
           foreach ($cart as $key2 => $cart2) {
                $start_datetime = new DateTime(date('H:i', strtotime('-4 hour', strtotime($cart_data[$key][$key2]['created_at']))));
                $end_datetime = new DateTime(date('H:i'));
                array_push($times, $date1->diff($date2) ($start_datetime - $end_datetime));
