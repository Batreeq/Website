    ...
    public function getUserCart(Request $request)
    {
        ...
        foreach ($cart_data as $key => $cart) {
           ...
           foreach ($cart as $key2 => $cart2) {
               if($cart2["status"] == 'confirmed' && strtotime(date('Y-m-d H:i:s' ,$cart2["created_at"])) > strtotime(date('Y-m-d H:i:s', strtotime('+4 hours')))){
                    return "true";
               } else {
                   return strtotime(date('Y-m-d H:i:s' ,$cart2["created_at"])) > strtotime(date('Y-m-d H:i:s', strtotime('+4 hours')));
               }
