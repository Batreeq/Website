    ...
    public function getUserCart(Request $request)
    {
        ...
        foreach ($cart_data as $key => $cart) {
           foreach ($cart as $key2 => $cart2) {
                ...
                if($start_datetime->diff($end_datetime)->y == 0 && $start_datetime->diff($end_datetime)->m == 0 )
                $h = $start_datetime->diff($end_datetime)->h;
                if($cart_data[$key][$key2]['status'] == 'confirmed' && $h > 4){
