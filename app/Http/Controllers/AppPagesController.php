...
class AppPagesController extends Controller
{
    ...
    function submit_update(Request $request){
        ...
        if(isset(request()->image)){
            ...
            $image = Terms::where('id', $request->id)->first()->image;
        }
    ...
    function submit_update_privacy_policy(Request $request){
        ...
        if(isset(request()->image)){
            ...
            $image = PrivacyPolicy::where('id', $request->id)->first()->image;
        }
    ...
    function submit_add_help(Request $request){
        ...
        if(isset(request()->image)){
            $imageName = time().'.'.request()->image->getClientOriginalExtension();
            request()->image->move(public_path('images'), $imageName);
            $help->image = 'https://jaraapp.com/images/'. $imageName;
        }
    ...
    function submit_update_help(Request $request){
        ...
        if(isset(request()->image)){
            ...
            $image = Help::where('id', $request->id)->first()->image;
        }

        Help::where('id', $request->id)->update(['title' => $request->title,'text' => $request->text,'image' => $image,'updated_at'=>date("Y-m-d h:i:s")]);
